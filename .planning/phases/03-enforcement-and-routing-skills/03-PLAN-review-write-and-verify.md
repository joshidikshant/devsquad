...
depends_on:
  ...
  - 02-PLAN-research-and-draft-pipeline.md
files_modified:
...
bash plugin/skills/code-generation/scripts/generate-skill.sh --help 2>&1 | grep -q "Usage" && echo "help flag OK"
test -f plugin/commands/generate.md && echo "CGEN-05: generate command registered"</verify>
  <done>bash -n passes on the complete script. grep confirms read -r prompt, write_state, delimiter parsing, and DRY_RUN guard are all present. --help prints usage. plugin/commands/generate.md exists.</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <name>Checkpoint: Human end-to-end verification</name>
  <action>Human manually executes verification steps listed in <how-to-verify>.</action>
  <what-built>
